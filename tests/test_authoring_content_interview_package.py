"""Package contract for master-cv-content-interview (issue #37).
...
_SKILL_DIR = _SKILLS_ROOT / "master-cv-content-interview"
...
def test_content_interview_skill_exists() -> None:
    assert _SKILL.is_file()


def test_content_interview_skill_is_user_invoked() -> None:
    text = _SKILL.read_text(encoding="utf-8")
    assert "name: master-cv-content-interview" in text
...
def test_content_interview_codex_sidecar_forbids_implicit_invocation() -> None:
    assert _SIDECAR.is_file()
    text = _SIDECAR.read_text(encoding="utf-8")
    assert "allow_implicit_invocation: false" in text


def test_content_interview_skill_points_at_the_authoring_write_protocol() -> None:
    text = _SKILL.read_text(encoding="utf-8")
...
def test_content_interview_skill_uses_portable_wording() -> None:
    text = _SKILL.read_text(encoding="utf-8")
