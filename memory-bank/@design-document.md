...
- **LLM Run** (Assessment Summary): shipped via `Assistant.start_catalog_assess_llm_run` / `start_bulk_prepare_llm_run` / `get_llm_run_status` / `stop_llm_run` + thin poll/Stop UI (parent #23); catalog assess drains until Pending empty (until-empty revisit of ADR-0015).
