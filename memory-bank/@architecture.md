...
- **Master CV Authoring:** `.agents/skills/master-cv-*` plus `.agents/skills/master-cv-write-protocol.md` (ADR-0018). Glossary and ADR-0018 name Holistic Slice, Detailed Slice, Facet, Capability bullet, and Gate met / Gate missed; the on-disk content-interview leaf still implements Detailed Slice only. `master-cv-design-pins` is named-write of a nominated Slice (ADR-0018 item 9): does not walk Facets, no required theme, no opening or closing layout audit, closing offer is silence, pin names resolve against the Sitting would-be `cv.sections`. Enrichment remains until Gate met. Assistant does not write the Master CV except Enrichment.
